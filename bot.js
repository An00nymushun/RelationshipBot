...
const UserSchema = new Mongoose.Schema({
	...
	pass: String
}, { versionKey: false }); //Mongoose functionality limited by this being disabled (save)
...
DiscordClient.on('ready', async () => {
	...
	if(Config.ERRORCHANNEL) {
		try {
			ErrorChannel = await DiscordClient.channels.fetch(Config.ERRORCHANNEL);
		}
...
const SetErrorChannelCommand = async (message) => {
	...
	Config.ERRORCHANNEL = ErrorChannel.id;
	try {
...
async function Reply(message, content, noMention) {
	try {
		...
		if((err.code  === 50001/*Missing Access*/ || err.code  === 50013/*Missing Permissions*/) && message.author != null && !message.author.bot) {
			await message.author.send(`I don't have permission to send messages in ${message.guild.name} <#${message.channel.id}>, please fix your server settings!\nThe original message was:`);
	...
	if(type === undefined) {
		if(!premium) {
			...
			if(lowerTypeName !== typeName)
				type = RelationshipMap.get(typeName.toLowerCase());
			if(type === undefined)
				Reply(message, `Sorry, ${typeName} relationship only works on a premium server (${Prefix}help premium)`).catch(LogError);
		}
		else type = typeName;
	}
	return type;
...
const RelationshipCommand = async (typeName, selfTypeName, message, commandBase) => {
	...
	for(let [mentionId] of mentions) {
		if(mentionId !== userId)
			SetRelationship(userId, mentionId, type, message);
	}
					...
					if(member !== undefined && !member.user.bot) {
						...
						if(RelationshipRequests.has(requestId)) {
							...
							try {
								...
								let requestMessage = await Reply(message, `<@${mentionId}>, you are ${initiatorName}'s ${typeName} now!\nWould you like them to be your ${selfTypeName + (type === selfType ? " too" : "")}?`, true);
...
const ShipCommand = async (message, command, commandBase) => {
	...
	let types = command.split(MentionsArrowsRegex).filter(x => x != "");
	if(types.length !== 2) {
		Reply(message, "Please specify the roles in the relationship in the command as:\nrole of the first person **->** role of the second person\nFor example: waifu -> husbando\nNote: they aren't server roles, just text").catch(LogError);
		return;
	}
...
function QueryRelationshipTree(userId, depth, withDepth) {
	return withDepth ? User.aggregate([{ "$match": { "_id": Types.Long.fromString(userId) } }])
		.graphLookup({
			from: "users",
			startWith: "$fam.id",
...
function SetUserNode(userNamePromises, promiseCounter, node, userId, fontSizeMult = 1) {
	promiseCounter[0]++;
	userNamePromises.push(DiscordClient.users.fetch(userId).then((user) => {
		let name = user.username;
		node.set('label', DotEscape(name))
			.set('fontsize', (30 - (name.length / 2)) * fontSizeMult);
		promiseCounter[0]--;
	}));
}
...
async function GetRelationshipTree(userId, depth = 2, trimEnds) {
	...
	if(query.tree !== undefined) {
		...
		for(let i = ids.length - 1; i >= 0; i--) {
			...
			ids[i] = id;
			if(!directSet.delete(id)) { //no duplicates so we can delete, returns true if exists
		...
		for(let i = ids.length - 1; i >= 0; i--) {
			...
			for(let relationship of fams[i]) {
				...
				if(typeof relationship.t === 'number') {
					...
					if(color == null) 
						g.addEdge(node, otherNode);
					else
						g.addEdge(node, otherNode, {color});
				}
				else {
...
const DirectRelationshipsCommand = async (message) => {
	...
	try {
		...
		let fields = Array.from(relationships, ([name, relationships]) => ({ //TODO fix overflows
			name: (typeof name === 'number') ? RelationshipIndex[name] : name,
			value: `<@${relationships.map(x => x.id).join("> <@")}>`
		}));


		await Reply(message, { embeds: [{
			fields
		}]});
	}
	catch(err) { LogError(err) }
...
const RelationshipGraphCommand = async (message, command, commandBase) => {
	...
	try {
		...
		let fetchStat;
		if(oldFetchStat != null)
			fetchStat = (oldFetchStat * StatisticsWeight + waitTime) / (StatisticsWeight + 1);
		else
			fetchStat = waitTime;
			
		Shard.broadcastEval(
			(client, { fetchedUsers, fetchStat }) => client.Statistics.fetch[fetchedUsers] = fetchStat,
			{ context: { fetchedUsers, fetchStat } });
		...
		let graphvizError = "";
		graphviz.stderr.on('data', (data) => { graphvizError += data });
		graphviz.on('exit', async (code) => {
			if(code !== 0) {
				...
				try {
					...
					let newStat;
					if(oldGraphStat != null)
						newStat = (Statistics.graph[graphSize] * StatisticsWeight + waitTime) / (StatisticsWeight + 1);
					else
						newStat = waitTime;

					Shard.broadcastEval(
						(client, { graphSize, newStat }) => client.Statistics.graph[graphSize] = newStat,
						{ context: { graphSize, newStat } });
...
const PictureEditCommand = async (message, command, commandBase, pictureName) => {
	...
	if(pictureName == null) {
		if(command.length !== commandBase.length) {
			...
			if(match != null)
				pictureName = Images.anipics[Images.animap.get(match[0].toLowerCase())];
		}
	}
	...
	try {
		...
		if(message.mentions.users.size !== 0) {
			...
			while(true) {
				await Promise.allSettled(fam.slice(lastTriedAvatars, triedAvatars).map(async (famId) => {
					await CdnDownloadSemaphoreWait();
					try {
				...
				if(avatars.length === requiredAvatars || triedAvatars === fam.length)
					break;
		...
		let {data, info} = await Sharp(await layers[0]).composite(extraLayers).raw().toBuffer({resolveWithObject: true});
...
async function FindPictureRectangles(imageBuffer) {
	...
	for (let yi = 0; yi < ySearchEnd; yi += minPfpSize) {
		...
		for (let xi = 0; xi < xSearchEnd; xi += minPfpSize) {
			if((data.readInt32LE(yStart + xi * channels) & 0x00FFFFFF) === 0x252220)
				spottedPixels.push((yi << 16) | xi);
		}
	}
	...
	for (const pixelCoords of spottedPixels) {
		...
		if(w >= minPfpSize && h >= minPfpSize)
			spottedRects.push(rect);
	}
	...
	switch (extension) {
		case 'zip':
			try {
				...
				Shard.broadcastEval(
					(client, { lowerFilename, fileName }) => client.Images.animap.set(lowerFilename, client.Images.anipics.push(fileName) - 1),
			...
			finally { CdnDownloadSemaphoreRelease(); }
...
const PictureDeleteCommand = async (message, command, commandBase) => {
	...
	try {
		...
		Images.animap = new Map(Images.anipics.map((name, i) => [name.toLowerCase(), i]));
		Shard.broadcastEval(
			(client, { lowerFilename }) => { let index = client.Images.animap.get(lowerFilename); if(index !== undefined) { client.Images.anipics.splice(index, 1); client.Images.animap = new Map(client.Images.anipics.map((name, i) => [name.toLowerCase(), i])); } },
			{ lowerFilename });

		let pictureData = JSON.parse(await Fs.readFile(`pics/anipics/metadata/${fileName}.json`, 'utf8'));
		
		if(typeof pictureData.l === 'string')
			await Fs.unlink(`pics/anipics/layers/${pictureData.l}`);
		else 
			await Promise.all(pictureData.l.map(x => Fs.unlink(`pics/anipics/layers/${x}`)));
...
const ClearRelationshipBlocksCommand = (message) => {
	User.updateMany({ "bloc": message.author.id }, { 
        "$pull": {
            "bloc": message.author.id
        }
	}).exec();
...
const ClearRate = (id) => { RateMeasurements.delete(id) };
